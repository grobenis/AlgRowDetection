...
int sample_num = 0;			//样本点序数
int16_t Wave[WAVELEN];		//波形
TempStack temp;				//中间变量
RawDataStack RawData;		//实时数据栈
ResultStruct RowResult;		//结果
DataStack Acc;
DataStack Ang;
//初始化算法参数
void initRowAlgoParameters(char handside) {
...
char rowing_receiveAccGyro(int16_t* acc_buf, int16_t* gyro_buf)
{
		...
		if (sample_num - 1 - temp.ValleyLoc >= 5)
		{
			...
			if ((PPdis >= 20 && PPdis <= temp.CycleLimit) || temp.LastPeakLoc == 0)
			{
				...
				if (temp.IsolatedPointsNumber > 3)
				{
					...
					RowResult.avgPaddleTime = RowResult.avgPaddleTime * temp.Count1 / (temp.Count1 + 1) + TempTime / (temp.Count1 + 1);
					...
					RowResult.rowCounts++;
					RowResult.avgRowCountPerMin = (int)(60 / RowResult.avgRowDuration + 0.5);
...
//获取结果 打印数据
void getRowingResult(ResultStruct* result) {
	*result = RowResult;
}
void printResult(ResultStruct RowResult)
{
	printf("rowCounts = %d\t", RowResult.rowCounts);
	printf("avgRowDuration = %.2f\t", RowResult.avgRowDuration);
	printf("fre = %d\t", RowResult.avgRowCountPerMin);
...
int16_t GetArrayMean(int16_t* Data, int16_t len)
{
	int16_t mean = 0;
	for (int i = 0; i < len; i++)
	{
...
//计算相似性
float ComputeSimilarity(int16_t* Array, int Num1, int16_t* Wave)
{
	float mean1 = 0;
	float mean2 = 0;

	mean1 = GetArrayMean(Array, Num1);
